...
export const CrudTareaPost = async (tareaData) => {
  try {
    const response = await axios.post(`${API_URL}/crud/tareas/`, tareaData, {
      headers: {
...
export const CrudTareaGet = async () => {
  try {
    const response = await axios.get(`${API_URL}/crud/tareas/`);
    return response.data;
  } catch (error) {
    console.error(
...
export const BuscarTareaPorId = async (pk) => {
  const response = await axios.get(`${API_URL}/crud/tareas/${pk}/`);
  return response.data;
};
